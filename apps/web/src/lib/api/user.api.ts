...
export const updateCurrentUser = async (updateData: Partial<User['details']>) => {
  try {
    const currentState = useUserStore.getState();
    const currentUser = currentState.users[currentState.activeUserIndex];

    if (!currentUser?.auth.accessToken) {
      throw new Error("Aucun token d'authentification trouvé");
    }

    const { id: userId } = jwtDecode<{ id: number }>(currentUser.auth.accessToken);

    const updatedUser = await api<User['details']>({
      method: 'PATCH',
      url: `/users/${userId}`,
      body: updateData,
      token: currentUser.auth.accessToken,
    });

    useUserStore.setState((state) => {
      const updatedUsers = state.users.map((user) => {
        if (user.details.id === userId) {
          return {
            ...user,
            details: updatedUser,
          };
