...
export class Analytics {
  ...
  static async trackWidgetUpdate(roomId: string | null, widgetState: { showIframe?: boolean }) {
    if (!ENABLED) return;

    if ('showIframe' in widgetState) {
      const eventName = widgetState.showIframe ? 'expand_link_widget' : 'collapse_link_widget';
